...
<template>
  <aside class="w-56 bg-rf-bg-light border-r border-rf-secondary/30 min-h-[calc(100vh-56px)]">
    <nav class="p-4">
      <ul class="space-y-2">
        <li v-for="item in navItems" :key="item.path">
          ...
          >
            <span class="mr-3 text-lg">
              ...
              <template v-else-if="item.icon === 'artifacts'">&#128230;</template>
              <template v-else-if="item.icon === 'diff'">&#8801;</template>
              <template v-else-if="item.icon === 'approvals'">&#10003;</template>
